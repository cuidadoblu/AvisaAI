						...
						<c:choose>
							<c:when test="${not empty incidente.fotosIncidente}">
								<c:forEach var="foto" items="${incidente.fotosIncidente}">
									<div class="item-foto">
										<img
											src="exibir-foto-incidente?id-incidente=${incidente.id}&foto-id=${foto.id}"
											alt="Imagem do incidente" class="imagem-postagem">
									</div>
								</c:forEach>
							</c:when>
							<c:otherwise>
								<div class="mensagem-sem-imagem">Este incidente não possui
									imagens.</div>
							</c:otherwise>
						</c:choose>
					</div>
					<div class="miniaturas">
						<c:if test="${not empty fotosIncidente}">
							<c:forEach var="foto" items="${fotosIncidente}">
								<img src="exibir-foto?id-foto=${foto.id}"
									alt="Imagem do incidente"
									onclick="trocarImagemPrincipal(${foto.id})"
